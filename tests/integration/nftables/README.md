...
`--cap-drop ALL` 与唯一补回的 `CAP_NET_ADMIN`；容器只读，`/run` 与 `/tmp`
...
全部 Guard-owned nftables 对象，并在退出前验证 foreign 对象保持不变。它不
冻结生产 hook/priority，也不替代目标 Linux、UFW 或 Docker 的 Golden State 验证。
