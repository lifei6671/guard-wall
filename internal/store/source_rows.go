...
type sourceRow struct {
	ActiveSessionID *string `gorm:"column:active_session_id"`                        // 当前写入 session；历史来源可空。
...
type sourceCheckpointRow struct {
	CheckpointSessionID *string `gorm:"column:checkpoint_session_id"`                    // 水位所属 session；迁移前记录可空。
	SourceID            string  `gorm:"column:source_id;primaryKey;autoIncrement:false"` // 来源身份主键。
...
type sourceFileGenerationRow struct {
	DurableEndOffset    *int64  `gorm:"column:durable_end_offset"`                        // 已证明的连续字节前缀；NULL 为未知。
	CoverageSessionID   *string `gorm:"column:coverage_session_id"`                       // 最近建立或推进前缀的 session。
