...
状态：Accepted，2026-09-04（D-016）；接口最小实施包已获确认。保留策略D-015保持。

## 接入位置

复用RunSourceIntakeRuntime现有所有权，不建立新排空服务：

Reader结束全部sink调用 → Seal队列 → worker正常退出 → Flush成功 → 同步维护 → Close数据库。

当前依据：internal/processor/source_intake_runtime.go:93-164及source/reader.go的返回契约。
...
RunSourceIntakeRuntime的最后增加一个参数：
...
本次仅拟修改带Reader所有权的RunSourceIntakeRuntime，RunSourceRuntime保持原接口。
...
- 任一前置错误、超时或Flush失败都跳过维护，沿现有路径传播错误和处理Close。
- 维护返回错误且deadline未过期时仍执行Close，合并维护错误与Close错误。
- 维护返回后deadline已过期，沿现有超时语义返回而不Close；进程所有者必须退出，不复用Store。
...
仅实施上述参数、内部传递与调用顺序，直接调用者适配nil，补充确定性测试。
